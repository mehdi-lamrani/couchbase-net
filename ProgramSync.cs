...
namespace Couchbase.Net.DevGuide
{
    class ProgramSync
    {
        ...
        private static void TestReplicatorConflictResolver()
        {
            ...
            var replConfig = new ReplicatorConfiguration(_Database, target);
        ...
        private static void TestSaveWithConflictHandler()
        {
            ...
            using (var document = _Database.GetDocument("xyz"))
            using (var mutableDocument = document.ToMutable())
            {
                mutableDocument.SetString("name", "apples");
                _Database.Save(mutableDocument, (updated, current) =>
                {
        ...
        private static void SelectAll()
        {
            ...
            using (var query2 = QueryBuilder.Select(SelectResult.All())
                .From(DataSource.Database(db)))
            {
